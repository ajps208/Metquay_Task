...
function EventsView({ sports }) {
  // Determine if the component is rendering sports events
  const isSports = sports ? true : false;
  
  // State to store sports events data
  const [sportEvent, setSportEvent] = useState([]);
  
  // State to store other events data
  const [otherEvent, setOtherEvent] = useState([]);
  
  // State to manage the search key for filtering events
  const [searchKey, setSearchKey] = useState("");
  
  // Fetch data based on the category (sports or others) and search key
  useEffect(() => {
    const fetchData = async () => {
      try {
        let result;
        // Fetch sports events if the component is rendering sports events
        if (isSports) {
          result = await allSportsAPI(searchKey);
          if (result.status === 200) {
            // Set sports event data if the API call is successful
            setSportEvent(result.data);
          } else {
            console.log(result);
          }
        } else {
          // Fetch other events if the component is rendering other events
          result = await allOtherEventAPI(searchKey);
          if (result.status === 200) {
            // Set other event data if the API call is successful
    ...
    // Call the fetchData function when the component mounts or when isSports or searchKey changes
    fetchData();
  }, [isSports, searchKey]);
  ...
  return (
    ...
    >
      ...
      <Container>
        <h3 className="mt-4 " style={{ color: "#9c7945" }}>
          {" "}
          {isSports ? (
            <>
        ...
        {isSports ? (
          <div className="pt-4 d-flex flex-row flex-wrap">
            <div
              ...
              style={{ width: "110px", borderRadius: "5px", color: "#8c8c8c" }}
          ...
          <div className="pt-4 d-flex flex-row flex-wrap">
            ...
            <div
              ...
              style={{ width: "115px", borderRadius: "5px", color: "#8c8c8c" }}
              className="border d-flex justify-content-center align-items-center m-1"
            >
            ...
            <div
              ...
              style={{ width: "150px", borderRadius: "5px", color: "#8c8c8c" }}
              className="border d-flex justify-content-center align-items-center m-1"
            >
